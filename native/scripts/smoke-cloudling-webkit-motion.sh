...
SMOKE_ROOT="${1:-${TMPDIR:-/tmp}/clawd-native-cloudling-webkit-motion-$RUN_ID}"
APP_PID=""
CURRENT_LOG=""
CURRENT_PORT=""
...
post_carrying() {
  curl -fsS \
    ...
    --data '{"state":"carrying","session_id":"native-cloudling-webkit-motion","event":"StateMatrix","agent_id":"codex","session_title":"cloudling webkit motion","cwd":"/tmp/clawd-native-cloudling-webkit-motion"}' \
    >/dev/null || fail "could not inject carrying state"
}
...
start_native
post_carrying
wait_current_state "carrying"
sleep 2.6
capture_window "$SMOKE_ROOT/cloudling-carrying-a.png"
sleep 1.2
